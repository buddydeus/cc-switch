...
impl AgentService {
    ...
    pub async fn check_updates(&self, db: &Arc<Database>) -> Result<Vec<AgentUpdateInfo>> {
        let agents = db.get_all_installed_agents()?;
        let mut updates = Vec::new();
        let mut repo_groups: HashMap<(String, String, String), Vec<InstalledAgent>> =
            HashMap::new();

        for agent in agents.into_values() {
            let (owner, name, branch) =
                match (&agent.repo_owner, &agent.repo_name, &agent.repo_branch) {
                    (Some(o), Some(n), Some(b)) => (o.clone(), n.clone(), b.clone()),
                    (Some(o), Some(n), None) => (o.clone(), n.clone(), "main".to_string()),
                    _ => continue,
                };
            if Self::repo_relative_path_from_id(&agent.id).is_none() {
                continue;
            }
            repo_groups
        ...
        for ((owner, name, branch), group_agents) in &repo_groups {
            let repo = SkillRepo {
                owner: owner.clone(),
                name: name.clone(),
                branch: branch.clone(),
            ...
            .await
            {
                Ok(Ok(result)) => result,
                Ok(Err(e)) => {
                    log::warn!("检查 Agent 更新时下载 {}/{} 失败: {e}", owner, name);
                    continue;
                }
                Err(_) => {
                    log::warn!("检查 Agent 更新时下载 {}/{} 超时", owner, name);
                    continue;
                }
            };

            for agent in group_agents {
                let Some(rel_path) = Self::repo_relative_path_from_id(&agent.id) else {
                    continue;
                };
                ...
                let remote_hash = match Self::compute_file_sha256(&remote_path) {
                    Ok(h) => h,
                    Err(e) => {
                        log::warn!("计算远程 Agent 哈希失败 {}: {e}", agent.id);
                        continue;
                    }
                };

                let local_path = ssot_dir.join(&agent.directory);
                let local_hash = match &agent.content_hash {
                    Some(h) => Some(h.clone()),
                    None => {
                        if local_path.is_file() {
                            match Self::compute_file_sha256(&local_path) {
                                Ok(h) => {
                                    let _ = db.update_agent_hash(&agent.id, &h, 0);
                                    Some(h)
                                }
                                Err(_) => None,
                            }
    ...
    pub async fn update_installed_agent(
        ...
        let rel_path = Self::repo_relative_path_from_id(&agent.id)
            .ok_or_else(|| anyhow!("Cannot update local-only agent: {agent_id}"))?;

        let (owner, name, branch) = match (&agent.repo_owner, &agent.repo_name) {
            (Some(o), Some(n)) => (
                o.clone(),
                n.clone(),
                agent
            ...
            _ => return Err(anyhow!("Cannot update agent without repo: {agent_id}")),
        };
        ...
        if !source.is_file() {
            let _ = fs::remove_dir_all(&temp_dir);
            return Err(anyhow!(
                "Agent file not found in repo: {}",
                source.display()
            ));
        }

        let _ = Self::create_uninstall_backup(&agent)?;

        let ssot_dir = Self::get_ssot_dir()?;
        let dest = ssot_dir.join(&agent.directory);
        fs::copy(&source, &dest)?;
        let _ = fs::remove_dir_all(&temp_dir);

        let new_hash = Self::compute_file_sha256(&dest).ok();
        let description = Self::extract_md_description(&dest).or(agent.description.clone());
        let readme_url = Some(Self::build_agent_doc_url(
            &owner,
            &name,
            &used_branch,
        ...
        let updated_agent = InstalledAgent {
            id: agent.id.clone(),
            name: agent.name.clone(),
            description,
            ...
            readme_url,
            apps: agent.apps.clone(),
            installed_at: agent.installed_at,
            content_hash: new_hash,
            updated_at: Utc::now().timestamp(),
        };

        db.save_agent(&updated_agent)?;

        for app in updated_agent.apps.enabled_apps() {
            if let Err(e) = Self::sync_file_to_app(&updated_agent.directory, &app) {
                log::warn!("同步更新后的 agent 到 {:?} 失败: {e}", app);
            }
        }

        log::info!("Agent {} 更新成功", updated_agent.name);
        Ok(updated_agent)
    }
