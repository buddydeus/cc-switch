...
use crate::services::skill::{SkillRepo, SkillService, SyncMethod};
...
impl RuleService {
    ...
    pub async fn install(
        ...
        let content_hash = Self::compute_file_sha256(&dest).ok();
    ...
    pub fn restore_from_backup(
        ...
        restored.content_hash = Self::compute_file_sha256(&restore_path).ok();
    ...
    /// 从统一 id（`owner/repo:relative/path.md`）解析仓库内相对路径；`local:` 返回 None
    fn repo_relative_path_from_id(id: &str) -> Option<String> {
        if id.starts_with("local:") {
            return None;
        }
        id.split_once(':').map(|(_, path)| path.to_string())
    }

    /// 检查所有已安装 Rule 的更新（有仓库来源且非 local）
    pub async fn check_updates(&self, db: &Arc<Database>) -> Result<Vec<RuleUpdateInfo>> {
        let rules = db.get_all_installed_rules()?;
        let mut updates = Vec::new();
        let mut repo_groups: HashMap<(String, String, String), Vec<InstalledRule>> =
            HashMap::new();

        for rule in rules.into_values() {
            let (owner, name, branch) =
                match (&rule.repo_owner, &rule.repo_name, &rule.repo_branch) {
                    (Some(o), Some(n), Some(b)) => (o.clone(), n.clone(), b.clone()),
                    (Some(o), Some(n), None) => (o.clone(), n.clone(), "main".to_string()),
                    _ => continue,
                };
            if Self::repo_relative_path_from_id(&rule.id).is_none() {
                continue;
            }
            repo_groups
        ...
        let ssot_dir = Self::get_ssot_dir()?;
        let skill_svc = SkillService::new();

        for ((owner, name, branch), group_rules) in &repo_groups {
            let repo = SkillRepo {
                owner: owner.clone(),
                name: name.clone(),
                branch: branch.clone(),
            ...
            let (temp_dir, _used_branch) = match timeout(
                std::time::Duration::from_secs(60),
                skill_svc.download_repo_pub(&repo),
            )
            .await
            {
                Ok(Ok(result)) => result,
                Ok(Err(e)) => {
                    log::warn!("检查 Rule 更新时下载 {}/{} 失败: {e}", owner, name);
                    continue;
                }
                Err(_) => {
                    log::warn!("检查 Rule 更新时下载 {}/{} 超时", owner, name);
                    continue;
                }
            };

            for rule in group_rules {
                let Some(rel_path) = Self::repo_relative_path_from_id(&rule.id) else {
                    continue;
                };
                let remote_path = temp_dir.join(&rel_path);
                if !remote_path.is_file() {
                    continue;
                }
                let remote_hash = match Self::compute_file_sha256(&remote_path) {
                    Ok(h) => h,
                    Err(e) => {
                        log::warn!("计算远程 Rule 哈希失败 {}: {e}", rule.id);
                        continue;
                    }
                };

                let local_path = ssot_dir.join(&rule.directory);
                let local_hash = match &rule.content_hash {
                    Some(h) => Some(h.clone()),
                    None => {
                        if local_path.is_file() {
                            match Self::compute_file_sha256(&local_path) {
                                Ok(h) => {
                                    let _ = db.update_rule_hash(&rule.id, &h, 0);
                                    Some(h)
                                }
                                Err(_) => None,
                            }
    ...
    /// 更新单个 Rule（从仓库重新拉取对应 .md）
    pub async fn update_rule(&self, db: &Arc<Database>, rule_id: &str) -> Result<InstalledRule> {
        let rule = db
            .get_installed_rule(rule_id)?
            .ok_or_else(|| anyhow!("Rule not found: {rule_id}"))?;

        let rel_path = Self::repo_relative_path_from_id(&rule.id)
            .ok_or_else(|| anyhow!("Cannot update local-only rule: {rule_id}"))?;

        let (owner, name, branch) = match (&rule.repo_owner, &rule.repo_name) {
            (Some(o), Some(n)) => (
                o.clone(),
                n.clone(),
                rule.repo_branch
            ...
            _ => return Err(anyhow!("Cannot update rule without repo: {rule_id}")),
        };
        ...
        let source = temp_dir.join(&rel_path);
        if !source.is_file() {
            let _ = fs::remove_dir_all(&temp_dir);
            return Err(anyhow!("Rule file not found in repo: {}", source.display()));
        }

        let _ = Self::create_uninstall_backup(&rule)?;

        let ssot_dir = Self::get_ssot_dir()?;
        let dest = ssot_dir.join(&rule.directory);
        fs::copy(&source, &dest)?;
        let _ = fs::remove_dir_all(&temp_dir);

        let new_hash = Self::compute_file_sha256(&dest).ok();
        let description = Self::extract_md_description(&dest).or(rule.description.clone());
        let readme_url = Some(Self::build_rule_doc_url(
            &owner,
            &name,
            &used_branch,
        ...
        let updated_rule = InstalledRule {
            id: rule.id.clone(),
            name: rule.name.clone(),
            description,
        ...
        db.save_rule(&updated_rule)?;

        for app in updated_rule.apps.enabled_apps() {
            if let Err(e) = Self::sync_file_to_app(&updated_rule.directory, &app) {
                log::warn!("同步更新后的 rule 到 {:?} 失败: {e}", app);
            }
        }

        log::info!("Rule {} 更新成功", updated_rule.name);
        Ok(updated_rule)
    }
