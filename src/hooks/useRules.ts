...
export function useCheckRuleUpdates() {
  return useQuery({
    queryKey: ["rules", "updates"],
    queryFn: () => rulesApi.checkUpdates(),
    enabled: false,
    staleTime: 5 * 60 * 1000,
  });
}
...
export function useUpdateRule() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => rulesApi.updateInstalled(id),
    onSuccess: (updatedRule) => {
      queryClient.setQueryData<InstalledRule[]>(
        ["rules", "installed"],
        (oldData) => {
          if (!oldData) return [updatedRule];
          return oldData.map((r) =>
            r.id === updatedRule.id ? updatedRule : r,
          );
        },
      );
      queryClient.setQueryData<RuleUpdateInfo[]>(
        ["rules", "updates"],
        (oldData) => {
          if (!oldData) return oldData;
          return oldData.filter((u) => u.id !== updatedRule.id);
        },
      );
    },
  });
