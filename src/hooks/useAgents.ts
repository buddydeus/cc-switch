...
export function useCheckAgentUpdates() {
  return useQuery({
    queryKey: ["agents", "updates"],
    queryFn: () => agentsApi.checkUpdates(),
    enabled: false,
    staleTime: 5 * 60 * 1000,
  });
}
...
export function useUpdateAgent() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => agentsApi.updateInstalled(id),
    onSuccess: (updatedAgent) => {
      ...
      queryClient.setQueryData<AgentUpdateInfo[]>(
        ["agents", "updates"],
        (oldData) => {
          if (!oldData) return oldData;
          return oldData.filter((u) => u.id !== updatedAgent.id);
        },
      );
    },
  });
